	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			//
			/*세션 객체에서 제공하는 getRealPath 메서드를 사용
			 ...
			 * 매개변수 생성자로 생성
			 * MultipartRequest multiRequest = new MultipartRequest(request 객체, 저장할 폴더 경로, 용량 제한, 인코딩값, 파일명 수정객체);
			 * 
			 * 위 구문 한줄이 실행되는 순간 지정한 경로에 파일이 업로드 된다.
			 * 사용자가 올린 파일명은 그대로 해당 폴더에 업로드 하지 않는게 일반적이다 - 같은 파일 명이 있을 경우 덮어씌워질 경우도 있고
			 * 한글/특수문자가 포함된 파일명 경우는 업로드되는 서버에 따라 오류를 발생시킬 여지가 있기때문.
			 * 
			 ...
			 * rename 메서드를 정의하면 해당 작업을 할 수 있다. (내 방식대로)
			 * 
			 * 
			 */
			//기본 제공 파일명 변경 객체 사용
//			MultipartRequest multiRequest = new MultipartRequest(request, savePath, maxSize, "UTF-8", new DefaultFileRenamePolicy());
			...
			//3.DB에 저장할 정보들 추출하기
			//-카테고리번호, 제목, 내용, 작성자번호 추출하여 board 테이블에 insesrt하기
			...
			if(result>0) {
				...
				//실패시에는 업로드된 파일을 지워주는 작업이 필요하다(게시글은 없는데 업로드 파일이 자원을 쓰고있으니)
				if(at!=null) { //넘어온 파일이 있어서 객체가 생성됐다면
					//해당 파일 경로 잡아서 File객체 생성 후 delete메서드로 파일 삭제 작업
